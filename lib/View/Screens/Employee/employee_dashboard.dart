...
class _EmployeeDashBoardState extends State<EmployeeDashBoard> {
  ...
  Widget build(BuildContext context) {
    final double deviceHeight = MediaQuery.of(context).size.height;
    final double deviceWidth = MediaQuery.of(context).size.width;
    const bool isWeb = kIsWeb; // Determine if running on web

    final tabs = loading && _employeeAccount.manager
        ? [
            SizedBox(
              width: deviceWidth,
              height: deviceHeight * (isWeb ? 1.0 : 0.79),
              child: Column(
                children: [
                  const Text("Employees"),
                  SizedBox(
                    height: deviceWidth * (isWeb ? 0.01 : 0.03),
                  ),
                  SizedBox(
                    height: deviceHeight * (isWeb ? 0.95 : 0.7),
                    child: ListView.builder(
                      itemBuilder: (context, index) {
                        return Card(
                          child: ListTile(
                            leading: SizedBox(
                                height: deviceHeight * (isWeb ? 0.05 : 0.055),
                                child: const Icon(Icons.person)),
                            title: Text(
                                "Name: ${employeeAccounts[index].firstName}  Income Earned: ${employeeAccounts[index].amountReceived}"),
                            subtitle: Text(
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: const Icon(Icons.exit_to_app),
            onPressed: () => _logoutAction(context, deviceWidth),
          ),
        ],
      ),
      drawer: loading
                  ...
                  ListTile(
                    ...
                    onTap: () {
                      Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => const EmployeeTasks(),
                          ));
                    },
                  ),
                ],
      ...
      body: loading
          ? _employeeAccount.manager
              ? Padding(
                  padding: EdgeInsets.all(deviceWidth * (isWeb ? 0.01 : 0.04)),
                  child: tabs[_currentIndex],
                )
              : Padding(
                  padding: EdgeInsets.all(deviceWidth * (isWeb ? 0.01 : 0.04)),
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Text('Income Earned: ',
                              style: TextStyle(
                                  fontSize: deviceWidth * (isWeb ? 0.03 : 0.06),
                                  fontWeight: FontWeight.w400)),
                          SizedBox(
                            width: deviceWidth * (isWeb ? 0.005 : 0.01),
                          ),
                          Text(
                              '${_employeeAccount.amountReceived.toStringAsFixed(2)} \$',
                              style: TextStyle(
                                  fontSize: deviceWidth * (isWeb ? 0.04 : 0.07),
                                  fontWeight: FontWeight.w500)),
                        ],
                      ),
                      SizedBox(
                        height: deviceHeight * (isWeb ? 0.01 : 0.02),
                      ),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                        children: [
                      ...
                      SizedBox(
                        height: deviceHeight * (isWeb ? 0.005 : 0.01),
                      ),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                        children: [
                          Text('Team: ',
                              style: TextStyle(
                                  ...
                                  fontWeight: FontWeight.w300)),
                          SizedBox(
                            width: deviceWidth * (isWeb ? 0.01 : 0.03),
                          ),
                          Text(_employeeAccount.team,
                              style: TextStyle(
                                  fontSize:
                      ...
                      ElevatedButton(
                        ...
                        child: Text('Add a Task',
                            style: TextStyle(
                                fontSize: deviceWidth * (isWeb ? 0.02 : 0.04))),
                      ),
                    ],
                  ),
                )
          : const LoadingAnimation2(),
      bottomNavigationBar: loading && _employeeAccount.manager
          ? BottomNavigationBar(
              items: const [
                BottomNavigationBarItem(
                    icon: Icon(Icons.work), label: "Employees"),
                BottomNavigationBarItem(
                    icon: Icon(Icons.computer), label: "PC Providers"),
              ],
              currentIndex: _currentIndex,
              onTap: (index) {
                setState(() {
